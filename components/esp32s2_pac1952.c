...
esp_err_t pac1952_busy(){
	...
	while(1){
        ...
        i2c_master_write_byte(cmd, (PAC194X5X_CLIENT_ADDRESS_1) | WRITE_BIT, ACK_CHECK_EN);
        i2c_master_stop(cmd);
...
esp_err_t PAC194x5x_Write(PPAC194X5X_DEVICE_CONTEXT pdevice, uint8_t *pdata, uint8_t length){
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, pdevice->i2cAddress | WRITE_BIT, ACK_CHECK_EN);
    i2c_master_write(cmd,pdata,length, ACK_CHECK_EN);
    i2c_master_stop(cmd);
    esp_err_t _pac1952_stat = i2c_master_cmd_begin(i2c_num, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    // ESP_LOGI("PAC write","status=%d",_pac1952_stat);
	return (_pac1952_stat);

}

esp_err_t PAC194x5x_Read(PPAC194X5X_DEVICE_CONTEXT pdevice, uint8_t *pdata, uint8_t *plength){

    uint16_t _pac1952_stat;
    uint8_t len;
    
    len = *plength;
    // ESP_LOGI("I2C_read","byte length = %d",len);
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, pdevice->i2cAddress | READ_BIT, ACK_CHECK_EN);
    if(len > 1){
...
uint16_t PAC194x5x_GetVBUSn_AVG_reg(PPAC194X5X_DEVICE_CONTEXT pdevice, uint8_t channelNo, uint16_t* pregister_val){
    ...
    *pregister_val = readBuffer[0];
    *pregister_val = (*pregister_val << 8) + readBuffer[1];
      ESP_LOGI("Vbusavg","CH2 Vbus avg: %d",  *pregister_val);
    return errorCode;
}
...
uint16_t PAC194x5x_GetAlertEnable_reg(PPAC194X5X_DEVICE_CONTEXT pdevice, PPAC194X5X_ALERT_REGISTER pAlertEnable_reg){
    ...
    //read the register
    errorCode = PAC194x5x_Read(pdevice, readBuffer, &byteCount);
    ESP_LOGI("GET_AL","[0]:0x%X [1]=0x%X [2]=0x%X", readBuffer[0],readBuffer[1],readBuffer[3]);
    if(errorCode != PAC194X5X_SUCCESS) return errorCode;
...
uint16_t PAC194x5x_GetVIP_digital(PPAC194X5X_DEVICE_CONTEXT pdevice,uint8_t *pValue, uint8_t byteSize){
    ...
    uint16_t Vsense_ch1,VBus_ch1,Vsense_ch2,VBus_ch2;
